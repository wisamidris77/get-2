...
class GetInstance {
  ...
  /// Get all registered instances with their details
  /// Returns a list of all instances currently registered in the system
  List<Map<String, dynamic>> getAllInstances() {
    ...
    for (final entry in _singl.entries) {
      ...
      // Extract type and tag from key
      final type = _extractTypeFromKey(key);
      final tag = _extractTagFromKey(key);
      
      instances.add({
        'key': key,
        'type': type,
        'tag': tag,
        'isPermanent': factory.permanent,
        'isPrepared': !factory.isInit,
        'isLazy': factory.fenix,
        'isInit': factory.isInit,
        'isSingleton': factory.isSingleton ?? false,
        'isDirty': factory.isDirty,
        'size': _calculateInstanceSize(factory.dependency),
      });
    }
  ...
  String _extractTypeFromKey(String key) {
    // Keys are usually in format "TypeName" or "TypeName_tag"
    final parts = key.split('_');
    return parts.first;
  }
  
  /// Extract tag from key
  String? _extractTagFromKey(String key) {
    final parts = key.split('_');
    if (parts.length > 1) {
  ...
  /// Calculate approximate size of instance
  int _calculateInstanceSize(dynamic instance) {
    try {
      if (instance == null) return 0;
      final stringRepresentation = instance.toString();
      return stringRepresentation.length * 2;
    } catch (e) {
      return 1024;
    }
  }
